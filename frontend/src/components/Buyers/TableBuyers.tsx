...
import { fetch, deleteItem } from '../../stores/buyers/buyersSlice';
...
const TableSampleBuyers = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { buyers, loading, count } = useAppSelector((state) => state.buyers);
  ...
  const buyersPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Buyers was deleted!');
    }
  };
        ...
        <tbody>
          {buyers &&
            ...
            buyers.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/buyers/${item.id}`)}
              >
...
export default TableSampleBuyers;

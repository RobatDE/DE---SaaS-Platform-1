...
import { fetch, deleteItem } from '../../stores/growth/growthSlice';
...
const TableSampleGrowth = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { growth, loading, count } = useAppSelector((state) => state.growth);
  ...
  const growthPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Growth was deleted!');
    }
  };
        ...
        <tbody>
          {growth &&
            ...
            growth.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/growth/${item.id}`)}
              >
...
export default TableSampleGrowth;

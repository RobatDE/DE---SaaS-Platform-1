...
import { fetch, deleteItem } from '../../stores/personality/personalitySlice';
...
const TableSamplePersonality = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { personality, loading, count } = useAppSelector(
  ...
  const personalityPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Personality was deleted!');
    }
  };
      ...
      <table>
        ...
        <tbody>
          {personality &&
            ...
            personality.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/personality/${item.id}`)}
              >
...
export default TableSamplePersonality;

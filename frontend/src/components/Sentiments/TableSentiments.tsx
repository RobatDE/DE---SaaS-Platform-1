...
import { fetch, deleteItem } from '../../stores/sentiments/sentimentsSlice';
...
const TableSampleSentiments = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { sentiments, loading, count } = useAppSelector(
    (state) => state.sentiments,
  );
  ...
  const sentimentsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Sentiments was deleted!');
    }
  };
        ...
        <tbody>
          {sentiments &&
            ...
            sentiments.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/sentiments/${item.id}`)}
              >
...
export default TableSampleSentiments;

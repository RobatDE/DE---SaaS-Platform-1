...
import { fetch, deleteItem } from '../../stores/markets/marketsSlice';
...
const TableSampleMarkets = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { markets, loading, count } = useAppSelector((state) => state.markets);
  ...
  const marketsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Markets was deleted!');
    }
  };
        ...
        <tbody>
          {markets &&
            ...
            markets.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/markets/${item.id}`)}
              >

...
import {
  fetch,
  deleteItem,
} from '../../stores/advertisements/advertisementsSlice';
...
const TableSampleAdvertisements = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const { advertisements, loading, count } = useAppSelector(
  ...
  const advertisementsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Advertisements was deleted!');
    }
  };
    ...
    <>
      ...
      <table>
        ...
        <tbody>
          {advertisements &&
            ...
            advertisements.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/advertisements/${item.id}`)}
              >
      ...
      <div className='p-3 lg:px-6 border-t border-gray-100 dark:border-slate-800'>
        <div className='flex flex-col md:flex-row items-center justify-between py-3 md:py-0'>
          <BaseButtons>
            {advertisementsPaginated.map((page) => (
              <BaseButton
                key={page}
...
export default TableSampleAdvertisements;

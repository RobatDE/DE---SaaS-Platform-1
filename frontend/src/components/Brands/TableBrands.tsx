...
import { fetch, deleteItem } from '../../stores/brands/brandsSlice';
...
const TableSampleBrands = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { brands, loading, count } = useAppSelector((state) => state.brands);
  ...
  const brandsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
    if (id) {
      ...
      setIsModalTrashActive(false);
      notify('success', 'Brands was deleted!');
    }
  };
        ...
        <tbody>
          {brands &&
            ...
            brands.map((item: any) => (
              <tr
                key={item.id}
                onClick={() => router.push(`/brands/${item.id}`)}
              >

...
import { brandsInputSchema, brandsSchema } from '../entity/Brands';

export const tag = 'Brands';

export default async (app: FastifyInstance) => {
    const schema = TypeBox.Type.Object({
        q: TypeBox.Type.Optional(TypeBox.Type.Partial(brandsSchema, { description: 'Filter query', additionalProperties: false })),
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/brands', {
        // @ts-ignore
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(BrandsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase());
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/brands/count', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
            querystring: schema,
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(BrandsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase(), true);
        return {
            rows: items,
            count,
            isLastPage: (req.query.page + 1) * req.query.limit >= count,
    ...
    app.get<{
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'Find brands instance to link as relations',
            response: {
        ...
        const repo = getCustomRepository(BrandsRepository);
    ...
    const postPayload = TypeBox.Type.Object({
        data: brandsInputSchema,
    });
    app.post<{
        Body: TypeBox.Static<typeof postPayload>,
        Reply: TypeBox.Static<typeof brandsSchema>
    }>('/brands', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
    ...
    // @ts-ignore
        ...
        const entity = await getCustomRepository(BrandsRepository).findOne({
            where: {
                id: req.params.id,
            },
        ...
        return entity ? entity : reply.notFound('Brands not found');
    });

    const putSchema = TypeBox.Type.Object({
        id: TypeBox.Type.String({ format: 'uuid' }),
        data: TypeBox.Type.Partial(brandsInputSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
    ...
    app.delete<{
        ...
        const entity = await getCustomRepository(BrandsRepository).softDelete(req.params.id);
        if (!entity.affected) {
            reply.notFound('brands not found');
            return;
        }
